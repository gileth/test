    ...
    public void work() {
        ...
        for (final PubUser u2 : users) {
            ...
            if (u2.getParent() != null) {
                final List<Double> winSum = this.dao.findByHql("select coalesce(sum(inoutNum),0) from GcLotteryDetail where uid=:uid and createDate>:startDate and createDate<:endDate and inoutNum>0",   ImmutableMap.of("uid",u2.getId(), "startDate", startDate,"endDate", endDate));
                final Double sum1 = winSum.get(0);
                final List<Double> loseSum = this.dao.findByHql("select coalesce(sum(inoutNum),0) from GcLotteryDetail where uid=:uid and createDate>:startDate and createDate<:endDate and inoutNum<0", ImmutableMap.of("uid", u2.getId(),"startDate", startDate, "endDate", endDate));
            ...
            if (rate > 0.0) {
                ...
                final List<Object> pcCounts = this.dao.findByHql(pcCountHql,  ImmutableMap.of("uid", u2.getId(), "startDate", startDate, "endDate", endDate));
                final Double pcWater = Double.valueOf(pcCounts.get(0).toString());
                if (pcWater < 0.0) {
                    final Long count = this.dao.count("select count(*) from PcGameLog where uid =:uid and status <>0",ImmutableMap.of("uid", u2.getId()));
                ...
                if (u2.getId() == 156) {}
                final List<Object> proxySum = this.dao.findByHql(proxyHql,  ImmutableMap.of("uid", u2.getId(), "startDate",startDate, "endDate", endDate));
